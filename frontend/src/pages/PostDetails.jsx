...
const PostDetails = () => {
  ...
  const [loading, setLoading] = useState(true); // Initialized to true to avoid flicker
    ...
    const fetchPost = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setLoading(false);
          setError(false); // FIXED: Should be false on success
        }
      } catch (error) {
        setError(true);
        setLoading(false);
  ...
  return (
    <main className="p-3 flex flex-col max-w-6xl m-auto min-h-screen">
      ...
      {/* FIXED: Added null check protection here */}
